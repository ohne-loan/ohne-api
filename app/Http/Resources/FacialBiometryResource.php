...
class FacialBiometryResource extends JsonResource
{
    /**
    ...
    {
        $image = UserDocument::find($this->user_documents_id)->path;

        return [
            'image' => gcs_sign_url($image),
            'approved' => $this->success ? "Sim" : "Não",
            'reason' => $this->cause_result
        ];
    }
