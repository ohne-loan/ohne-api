...
class AlterTableLoansAlterStatuses extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::statement(@"ALTER TABLE loans ADD CONSTRAINT loans_status_check CHECK (status::text = ANY (ARRAY['PENDING_PRE_APPROVAL'::character varying::text, 'PENDING_APPROVAL'::character varying::text, 'WAIT_SIGN'::character varying::text, 'IN_PROGRESS'::character varying::text, 'FINISHED'::character varying::text, 'REJECTED'::character varying::text, 'REJECTED_BY_PARTNER'::character varying::text, 'PROCESSING'::character varying::text, 'USER_REGISTERING'::character varying::text, 'CANC_BY_CLIENT'::character varying::text, 'CANC_CONTR_NSENT'::character varying::text, 'RENEGOTIATION_IN_PROGRESS'::character varying::text]))");
    }
